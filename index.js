...
function setUrl(search, select) {
  ...
  if (typeof(select) === 'string') {
    if (select) opt.push('ftg='+select.replace(/^ftg-/,''));
  } else {
    if (data.fg) opt.push('ftg='+data.fg);
  }
  opt = opt.join('&');
  if (opt) url = url + '?' + opt;
...
function showIcon(icon, url) {
  if (icon && icon.code) {
    currentGlyph = icon;
    $('#icon i').removeClass().addClass('ftg-'+icon.name)
    $('#icon h2').text('ftg-' + icon.name);
...
function save(opt) {
  ...
  switch (opt) {
    ...
    default: {
      ...
      ctx.font  = opt + 'px font-tidop-gis';
...
$.ajax({
  ...
  success: function(font) {
    ...
    search(data.q);
    if (data.ftg) {
      showIcon(glyphs['ftg-'+data.ftg]);
      showPage('icons');
    } else {
      showIcon(glyphs['ftg-earth'], false);
    }
  }
});