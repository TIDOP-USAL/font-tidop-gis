...
var fontName = 'font-tidop-gis';    // set name of your symbol font
const className = 'ftg'        // set class name in your CSS
...
/** Get glyphs */
var font = JSON.parse(fs.readFileSync('./font-tidop-gis.json'));
...
gulp.task('store', function(){
  return gulp.src(['svg/**/u*.svg'])
    .pipe(rename(function (path) {
      path.dirname = '';
      path.basename = 'ftg-' + path.basename.replace(/u([^-]*)-/,'');
    }))
    .pipe(svgmin({
      plugins: [{
